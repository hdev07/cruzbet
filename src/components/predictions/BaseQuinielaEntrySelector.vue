...
<template>
  <div class="mb-4">
    ...
    <p v-if="formError" class="mb-2 rounded-lg bg-red-500/10 px-3 py-2 text-sm text-red-300">
      {{ formError }}
    </p>
    ...
    <div class="flex flex-wrap gap-2">
      <button
        ...
        :class="
          entry.entry_number === currentEntry
            ...
            : 'border-app-border bg-white/5 text-slate-300 hover:border-mundial-accent/40'
        "
      ...
      >
        ...
        <span
          ...
          class="ml-1 text-[0.65rem] font-normal text-app-muted"
        >
          · borrador
        </span>
      </button>
    ...
    <div
      ...
      class="mt-3 rounded-lg border border-app-border bg-black/20 px-3 py-2.5"
    >
      <div v-if="editingName" class="space-y-2">
        <label class="text-xs font-medium text-app-muted" :for="`entry-name-${currentEntry}`">
        ...
        <input
          ...
          maxlength="30"
          autocomplete="off"
          class="w-full rounded-lg border border-app-border bg-mundial-dark px-3 py-2 text-sm"
          :disabled="savingName"
        ...
        <div class="flex gap-2">
          ...
          <button
            type="button"
            class="inline-flex items-center gap-1 rounded-lg border border-app-border px-3 py-1.5 text-xs text-slate-300 hover:bg-white/5 disabled:opacity-50"
            :disabled="savingName"
      ...
      <div v-else class="flex items-center justify-between gap-2">
        <div class="min-w-0">
          <p class="text-xs text-app-muted">Nombre de esta quiniela</p>
        ...
        <button
          type="button"
          class="shrink-0 rounded-lg p-1.5 text-app-muted transition hover:bg-white/10 hover:text-slate-200"
