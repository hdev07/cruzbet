-- Reloj monotónico: no retroceder tras el 90' ni por max(minute) de goles en vivo
...
begin
  ...
  select coalesce(max(minute + coalesce(extra_time, 0)), 0) into max_min
  from match_events
  where match_id = p_match_id
    and event_type = 'goal';
