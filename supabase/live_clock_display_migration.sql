-- Reloj en vivo legible (45+2', HT, etc.) desde sync ESPN
...
begin
  ...
  if new_status = 'live' and m.status = 'scheduled' then
    update matches
    set status = 'live',
        current_minute = coalesce(p_current_minute, 0),
        live_clock_display = coalesce(p_live_clock_display, live_clock_display),
    ...
    update matches
    set current_minute = case
          when p_live_clock_display is not null then coalesce(p_current_minute, current_minute)
          else greatest(coalesce(current_minute, 0), coalesce(p_current_minute, 0))
        end,
        live_clock_display = coalesce(p_live_clock_display, live_clock_display),
