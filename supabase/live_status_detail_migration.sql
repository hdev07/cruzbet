-- Detalle de estado en vivo desde ESPN (retrasado, pospuesto, etc.)
...
revoke all on function public.apply_live_sync(uuid, text, integer, integer, integer, jsonb, text, text, jsonb, text) from public;
grant execute on function public.apply_live_sync(uuid, text, integer, integer, integer, jsonb, text, text, jsonb, text) to service_role;
